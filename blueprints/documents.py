...
def share_document(doc_id):
    ...
    if request.method == "POST":
        ...
        expiry_choice = request.form.get("expiry_choice")
        ...
        if expiry_choice == "custom":
            try:
                ...
                flash("Enter a valid custom expiry in minutes.", "danger")
                return redirect(url_for("documents.share_document", doc_id=doc.id))
        else:
            minutes = int(expiry_choice)
        ...
        flash(f"Share link created for {recipient.username}, valid until {expires_at} UTC.", "success")
        return render_template("share_result.html", share_url=share_url, share=share, doc=doc)

    return render_template(
        "share.html", doc=doc, users=users, expiry_options=current_app.config["EXPIRY_OPTIONS"]
    )
