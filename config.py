...
class Config:
    # --- Core Flask ---
    SECRET_KEY = os.environ.get("SECRET_KEY", "change-this-in-production-use-a-random-64-char-string")
    ...
    FERNET_KEY = os.environ.get("FERNET_KEY", "z3nQe8u1v6t2yX0pL7bR4sN9mK5cD8fA1hJ6gT3wZ2Y=")
    ...
    # --- Share link access window ---
    # Instead of short fixed durations (10 min / 30 min / 1 day / custom),
    # shared links are only openable during business hours each day, up until
    # the "valid through" date the owner picks when creating the link.
